...
var D=[26,26,46], P=[139,92,246], CY=[6,182,212], W=[255,255,255], GR=[100,100,130], LG=[240,240,250];
...
// Draw the letterhead image as header - spans full width
function drawLetterhead(doc, letterheadB64) {
  var pw = doc.internal.pageSize.width;
  // Letterhead image - full width, proportional height (original is ~1240x248px = ratio 5:1)
  var lhH = pw / 5.0;
  try {
    doc.addImage(letterheadB64, 'JPEG', 0, 0, pw, lhH);
  } catch(e) {
    // Fallback dark header if image fails
    doc.setFillColor(D[0],D[1],D[2]); doc.rect(0,0,pw,44,'F');
    doc.setFillColor(P[0],P[1],P[2]); doc.rect(0,40,pw,4,'F');
    doc.setFont('helvetica','bold'); doc.setFontSize(16); doc.setTextColor(W[0],W[1],W[2]);
    doc.text(CO.name.toUpperCase(), 14, 24);
    lhH = 44;
  }
  return lhH + 4;
}

// Document type label (QUOTATION / TAX INVOICE) overlaid top-right
function drawDocLabel(doc, type, y) {
  var pw = doc.internal.pageSize.width;
  // Dark pill label top right
  var lw = 52, lh = 10, lx = pw - lw - 6, ly = 6;
  doc.setFillColor(26,26,46);
  doc.roundedRect(lx, ly, lw, lh, 3, 3, 'F');
  doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(W[0],W[1],W[2]);
  doc.text(type, lx + lw/2, ly + 6.5, {align:'center'});
  return y;
}
...
export function generateQuotePDF(quote, letterheadB64) {
  var doc=new jsPDF({unit:'mm',format:'a4'}), pw=doc.internal.pageSize.width;
  var y = drawLetterhead(doc, letterheadB64);
  drawDocLabel(doc, 'QUOTATION', y);
  ...
  drawTotals(doc,rows,y,pw); y+=rows.length*9+12;
  ...
  if(y<256){
    ...
    doc.text('Designation: _______________________',pw/2+4,y); y+=8;
    doc.text('Signature: _______________________________',10,y);
    doc.text('Date: _______________________',pw/2+4,y); y+=8;
...
export function generateInvoicePDF(invoice, payments, letterheadB64) {
  if(!payments) payments=[];
  var doc=new jsPDF({unit:'mm',format:'a4'}), pw=doc.internal.pageSize.width;
  var y = drawLetterhead(doc, letterheadB64);
  drawDocLabel(doc, 'TAX INVOICE', y);
  ...
  if(y<248){
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.setTextColor(GR[0],GR[1],GR[2]);
    doc.text('Account Name: Information Networking',10,y); y+=5;
    doc.text('Please quote invoice number on all payments.',10,y); y+=5;
  }
  ...
  if(y<258){
    ...
    if(invoice.notes){doc.text(invoice.notes,10,y,{maxWidth:pw-20});}
  }

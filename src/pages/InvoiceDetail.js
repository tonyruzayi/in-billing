...
export default function InvoiceDetail(){
  ...
  async function addPayment(){
    ...
    const bal=Number(invoice.total)-(payments.reduce((s,p)=>s+Number(p.amount),0));
    if(Number(payForm.amount)>bal+0.01){toast.error(`Amount exceeds balance of BWP ${bal.toFixed(2)}`);return;}
    setSaving(true);
  ...
  function sendEmail(){
    if(!emailTo){toast.error('Enter email');return;}
    ...
    toast.success('Email client opened');setEmailModal(false);
  }

  if(!invoice)return<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'50vh',color:'var(--muted)'}}>Loading…</div>;
  ...
  return(<>
    <div className="page-hdr">
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        <button className="btn btn-ghost btn-icon" onClick={()=>nav('/invoices')}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button>
        <div><h1 className="page-title">{invoice.invoice_number}</h1><p className="page-sub">Tax Invoice · {invoice.client_name}</p></div>
      </div>
      <div className="btn-row">
        <button className="btn btn-secondary btn-sm" onClick={()=>exportInvoiceXLSX(invoice,payments)}>📊 Excel</button>
        <button className="btn btn-secondary btn-sm" onClick={()=>generateInvoicePDF(invoice,payments,LOGO)}>📄 PDF</button>
        ...
        {invoice.status!=='paid'&&<button className="btn btn-ok btn-sm" onClick={()=>setPayModal(true)}>💳 Record Payment</button>}
      </div>
    </div>
    ...
    <div className="page-body">
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:16}}>
        <div className="stat-card cyan"><div className="stat-lbl">Invoice Total</div><div className="stat-val sm">{fa(invoice.total)}</div></div>
        <div className="stat-card ok"><div className="stat-lbl">Amount Paid</div><div className="stat-val sm" style={{color:'var(--ok)'}}>{fa(totalPaid)}</div><div className="stat-sub">{payments.length} payment{payments.length!==1?'s':''}</div></div>
        <div className={`stat-card ${balance<=0?'ok':'warn'}`}><div className="stat-lbl">Balance Due</div><div className="stat-val sm" style={{color:balance<=0?'var(--ok)':'var(--warn)'}}>{balance<=0?'✓ PAID':fa(balance)}</div></div>
      </div>
      ...
      <div className="det-hdr">
        ...
        <span className={`badge b-${invoice.status||'unpaid'}`} style={{fontSize:'.8rem',padding:'5px 12px'}}>{invoice.status||'unpaid'}</span>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
        ...
        <div className="card">
          <div style={{fontSize:'.68rem',fontWeight:700,color:'var(--cyan)',letterSpacing:'.08em',textTransform:'uppercase',marginBottom:8}}>Bill To</div>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        <div className="tbl-wrap"><table>
          ...
          <tbody>{(invoice.items||[]).map((it,i)=><tr key={i}><td className="td-m">{i+1}</td><td>{it.description}</td><td style={{textAlign:'right'}}>{it.qty}</td><td style={{textAlign:'right'}}>{fa(it.unit_price)}</td><td style={{textAlign:'right',fontFamily:'Syne,sans-serif',fontWeight:600}}>{fa((Number(it.qty)||0)*(Number(it.unit_price)||0))}</td></tr>)}</tbody>
        </table></div>
        <div style={{marginTop:14}}>
          <div className="totals-panel">
            ...
            {payments.map((p,i)=><div key={p.id} className="tot-row pay"><span>Payment {i+1} ({fd(p.payment_date)})</span><span>- {fa(p.amount)}</span></div>)}
            {payments.length>0&&<div className={`tot-row ${balance<=0?'bal':'grand'}`}><span>BALANCE DUE</span><span>{balance<=0?'✓ FULLY PAID':fa(balance)}</span></div>}
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:14}}>
        ...
        {payments.length===0
          ?<div style={{textAlign:'center',color:'var(--muted)',padding:'20px 0',fontSize:'.84rem'}}>No payments recorded yet</div>
          :payments.map(p=><div key={p.id} className="pay-item">
            <div>
              <div style={{display:'flex',alignItems:'center',gap:9}}>
                <span className="pay-amt">{fa(p.amount)}</span>
                <span className="badge b-draft">{p.payment_method}</span>
              </div>
              <div style={{fontSize:'.75rem',color:'var(--muted)',marginTop:3}}>{fd(p.payment_date)}{p.reference?' · Ref: '+p.reference:''}{p.notes?' · '+p.notes:''}</div>
            </div>
            <button className="btn btn-ghost btn-icon btn-xs" onClick={()=>deletePayment(p.id)} title="Remove">🗑️</button>
          </div>)
        }
      </div>

      {invoice.notes&&<div className="card"><div className="card-title" style={{marginBottom:8}}>Notes</div><p style={{fontSize:'.84rem',color:'var(--muted)',lineHeight:1.7}}>{invoice.notes}</p></div>}
    </div>

    {/* Payment Modal */}
    {payModal&&<div className="modal-overlay" onClick={()=>setPayModal(false)}>
        ...
        <div className="form-grid" style={{gap:12}}>
          ...
          <div className="fg"><label>Payment Method</label>
            <select className="fc" value={payForm.payment_method} onChange={e=>setPayForm(f=>({...f,payment_method:e.target.value}))}>
              <option>Bank Transfer</option><option>Cash</option><option>Cheque</option><option>Mobile Money</option><option>Card</option><option>Other</option>
            </select>
          </div>
    ...
    {emailModal&&<div className="modal-overlay" onClick={()=>setEmailModal(false)}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <div className="fg"><label>Message</label><textarea className="fc" rows={9} value={emailMsg} onChange={e=>setEmailMsg(e.target.value)}/></div>
        <div className="alert alert-info" style={{marginTop:12,fontSize:'.78rem'}}>💡 Download the PDF first, then open your mail client to attach it.</div>
        <div className="modal-footer">
          <button className="btn btn-secondary" onClick={()=>generateInvoicePDF(invoice,payments,LOGO)}>📄 Download PDF first</button>
